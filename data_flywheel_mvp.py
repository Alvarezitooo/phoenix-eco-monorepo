...
class DataFlywheelMVP:
        ...
        try:
            ...
            if result:
                ...
                cursor.execute("""
                    ...
                    WHERE prompt_version = ? AND sector_combination = ?
                """, (usage_count, new_success_rate, datetime.now().isoformat(),
                      pattern.prompt_version, sector_combination))
            else:
                # Création
                cursor.execute("""
                    ...
                    VALUES (?, ?, 1, 1.0, 0.0, ?)
                """, (pattern.prompt_version, sector_combination, datetime.now().isoformat()))
    ...
    def get_best_prompt_for_profile(
        ...
        try:
            ...
            if result and result[2] >= 5:  # Au moins 5 utilisations
                logger.info(f"Prompt recommandé pour {sector_combination}: {result[0]} (success: {result[1]:.2f})")
                return result[0]
            else:
                logger.info(f"Pas assez de données pour {sector_combination}, utilisation prompt par défaut")
    ...
    def get_analytics_dashboard(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "prompt_performance": [
                    {"version": perf[0], "avg_success": perf[1], "combinations": perf[2]}
                    for perf in prompt_performance
                ],
                "user_distribution": [
                    {"tier": tier[0], "count": tier[1]}
    ...
    def export_learning_data(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "recommendations": self._generate_prompt_recommendations(learning_patterns),
    ...
    def _generate_prompt_recommendations(self, patterns: List[Dict]) -> List[str]:
        ...
        for combo, scores in sector_performance.items():
            ...
            if avg_score < 0.7:
                recommendations.append(f"Améliorer prompt pour {combo} (score: {avg_score:.2f})")
            elif avg_score > 0.9:
                recommendations.append(f"Excellent prompt pour {combo} - utiliser comme modèle")
...
class PhoenixDataFlywheelIntegration:
    ...
    def enhance_letter_generation(
        ...
        source_sector, target_sector = self.flywheel._extract_sectors(cv_text, job_offer)
        
        # Obtenir le meilleur prompt
        best_prompt = self.flywheel.get_best_prompt_for_profile(source_sector, target_sector)
