...
class SecureATSOptimizer:
    ...
    def analyze_ats_compatibility_secure(
        ...
        try:
            ...
            cv_content = self._build_cv_content_for_analysis(user_profile)

            prompt_data = {
                "cv_content": cv_content,
                "job_description": (
                    f"Offre d'emploi cible: {clean_job_desc}" if clean_job_desc else ""
                ),
            }

            response = self.gemini.generate_content_secure("ats_analysis", prompt_data)
    ...
    def _build_cv_content_for_analysis(self, user_profile: UserProfile) -> str:
        """Construction sécurisée du contenu CV pour analyse"""
        content_parts = []

        if user_profile.professional_summary:
            content_parts.append(f"Résumé: {user_profile.professional_summary}")

        if user_profile.target_position:
            content_parts.append(f"Poste visé: {user_profile.target_position}")
        ...
        if user_profile.skills:
            skills_text = ", ".join([skill.name for skill in user_profile.skills[:20]])
