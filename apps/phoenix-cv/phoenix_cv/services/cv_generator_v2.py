...
from models.cv_models import (
    AccrocheIA,  # Assumons que ces modèles existent
    ATSOptimization,
    Competence,
    CVRequest,
    CVResponse,
...
class CVGeneratorServiceV2:
    ...
    async def generer_cv_complet_async(
        self, cv_text: str, annonce_text: str, template_id: str
    ) -> CVResponse:
        ...
        accroche_task = asyncio.create_task(
            self._generer_accroche_personnalisee_async(request_anonyme)
        )
        experiences_task = asyncio.create_task(
            self._optimiser_experiences_async(request_anonyme)
        )
        ats_task = asyncio.create_task(
            self._optimiser_pour_ats_async(request_anonyme, annonce_text)
        )
        ...
        conseils = await self._generer_conseils_amelioration_async(
            request_anonyme, optimisation_ats.score_global
        )
        ...
        return CVResponse(
            ...
            competences_mises_en_avant=self._selectionner_competences_cles(
                request
            ),  # synchrone, car rapide
    ...
    async def _generer_accroche_personnalisee_async(
        self, request: CVRequest
    ) -> AccrocheIA:
        """Génère une accroche CV structurée (Pydantic) via l'IA."""
        prompt = self._build_prompt_accroche_reconversion(
            request
    ...
    def _build_prompt_accroche_reconversion(self, request: CVRequest) -> str:
        ...
        competences_str = (
            ", ".join(request.competences_transferables)
            if request.competences_transferables
            else "Non spécifiées"
        )
...
ACCROCHE CV :"""
    ...
    async def _optimiser_experiences_async(
        self, request: CVRequest
    ) -> List[Dict[str, Any]]:
        ...
        for exp in request.experiences:
            if request.est_reconversion:
                ...
                description_optimisee = (
                    await self._reformuler_experience_reconversion_async(
                        exp, request.nouveau_domaine, request.competences_transferables
                    )
                )
            else:
                # Optimisation ATS classique
                description_optimisee = self._optimiser_experience_ats(
                    exp, request.secteur_cible
                )
    ...
    async def _reformuler_experience_reconversion_async(
        ...
        competences_str = (
            ", ".join(competences_transferables) if competences_transferables else ""
        )
...
Reformule cette expérience pour {nouveau_domaine} :"""
    ...
    def _optimiser_experience_ats(
        self, experience: Experience, secteur_cible: str
    ) -> str:
    ...
    def _selectionner_competences_cles(self, request: CVRequest) -> List[str]:
        ...
        competences_techniques = [
            comp.nom
            for comp in request.competences
    ...
    async def _optimiser_pour_ats_async(
        self, request: CVRequest, annonce_text: str
    ) -> ATSOptimization:
    ...
    async def _generer_conseils_amelioration_async(
        self, request: CVRequest, score_ats: Optional[int] = None
    ) -> List[str]:
        ...
        if request.est_reconversion:
            conseils.append(
                "Mettez davantage l'accent sur vos compétences transférables dans l'accroche"
            )
            conseils.append(
                "Quantifiez vos réalisations passées pour prouver votre impact"
            )

        # Conseils basés sur le score ATS
        if score_ats and score_ats < 70:
            conseils.append(
                "Intégrez plus de mots-clés spécifiques à votre secteur cible"
            )
            conseils.append(
                "Utilisez des verbes d'action forts dans vos descriptions d'expériences"
            )

        # Conseils génériques pertinents
        if len(request.experiences) > 5:
            conseils.append(
                "Concentrez-vous sur vos 4-5 expériences les plus pertinentes"
            )

        if not request.centres_interet:
            conseils.append(
                "Ajoutez quelques centres d'intérêt qui montrent votre personnalité"
            )
