...
class AIOptimizationManager:
    ...
    def generate_optimized_content(
        ...
        try:
            ...
            ai_response = self._call_ai_with_optimizations(
                optimized_params, request.user_tier
            )
    ...
    def _apply_security_checks(self, request: GenerationRequest) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_threat": max(
                [r["threat_level"] for r in security_results], default="low"
            ),
        }

    def _apply_rag_personalization(
        self, request: GenerationRequest, user_context: Optional[Dict[str, Any]]
    ) -> str:
    ...
    def _apply_cost_optimization(
        self, prompt: str, user_tier: UserTier, endpoint: str
    ) -> Dict[str, Any]:
        ...
        optimization_result = self.cost_optimizer.optimize_request_parameters(
            prompt=prompt, user_tier=user_tier.value, endpoint=endpoint
        )

        # Accumuler économies
        self.optimization_metrics["cost_savings_usd"] += optimization_result[
            "savings_usd"
        ]
    ...
    def _call_ai_with_optimizations(
        self, optimized_params: Dict[str, Any], user_tier: UserTier
    ) -> str:
