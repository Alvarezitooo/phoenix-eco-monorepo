...
class APICostOptimizer:
    ...
    def estimate_request_cost(
        self, prompt: str, max_tokens: int = 1000, model: str = "gemini-1.5-flash"
    ) -> Dict[str, Any]:
        """
        ...
        output_cost = (estimated_output_tokens / 1000) * model_pricing[
            "output_token_cost"
        ]
    ...
    def optimize_request_parameters(
        ...
        for rule in sorted(
            self.optimization_rules, key=lambda r: r.priority, reverse=True
        ):
            if self._evaluate_rule_condition(
                rule, prompt, user_tier, max_tokens, endpoint
            ):
        ...
        optimized_estimate = self.estimate_request_cost(
            optimized_params["prompt"], optimized_params["max_tokens"]
        )

        actual_savings = (
            original_estimate["total_cost_usd"] - optimized_estimate["total_cost_usd"]
        )
        savings_percentage = (
            (actual_savings / original_estimate["total_cost_usd"]) * 100
            if original_estimate["total_cost_usd"] > 0
            else 0
        )
        ...
        return {
            ...
            "token_reduction": original_estimate["estimated_total_tokens"]
            - optimized_estimate["estimated_total_tokens"],
        }
    ...
    def _calculate_optimization_potential(
        self, total_tokens: int, prompt: str
    ) -> Dict[str, float]:
        ...
        if total_tokens > 1200:
            potential["token_reduction"] = min(
                0.25, (total_tokens - 1200) / total_tokens
            )
    ...
    def get_cost_analytics(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if user_id:
            ...
            return {
                ...
                "avg_cost_per_request": (
                    total_cost / len(user_metrics) if user_metrics else 0
                ),
        ...
        return {
            ...
            "avg_tokens_per_request": (
                sum(m.total_tokens for m in all_metrics) / len(all_metrics)
                if all_metrics
                else 0
            ),
    ...
    def _get_recent_usage(
        self, metrics: List[APIUsageMetrics], days: int
    ) -> Dict[str, Any]:
    ...
    def _get_top_expensive_endpoints(
