...
class MirrorMatchService:
    ...
    def analyze_company_culture(
        ...
        if user_tier == UserTier.FREE:
            logger.info(
                "Mirror Match is a premium feature. Returning default for Free user."
            )
        ...
        try:
            ...
            ai_analysis = self._get_ai_cultural_analysis(
                company_name, job_offer, additional_info, user_tier
            )
            ...
            logger.info(
                f"Culture analysis completed for {company_name} - Score: {culture.confidence_score}"
            )
            return culture
    ...
    def _analyze_keywords(self, job_offer: str) -> Dict:
        ...
        dominant_industry = (
            max(industry_scores.items(), key=lambda x: x[1])[0]
            if industry_scores
            else "general"
        )
        ...
        communication_style = (
            max(comm_scores.items(), key=lambda x: x[1])[0] if comm_scores else "formal"
        )
        ...
        return {
            ...
            "keyword_confidence": len(industry_scores)
            / 10.0,  # Score de confiance basique
        }

    def _get_ai_cultural_analysis(
        self,
        company_name: str,
        job_offer: str,
        ...
        try:
            response = self.ai_client.generate_content(
                prompt=prompt, temperature=0.3, max_tokens=500, user_tier=user_tier
            )
    ...
    def _merge_analyses(
        self, company_name: str, basic: Dict, ai: Dict
    ) -> CompanyCulture:
        ...
        return CompanyCulture(
            ...
            tone_recommendations=ai.get(
                "tone_recommendations", ["Ton professionnel adapté"]
            ),
            ...
            confidence_score=min(
                basic.get("keyword_confidence", 0.3) + ai.get("confidence_score", 0.3),
                1.0,
            ),
        )
    ...
    def _detect_company_size(self, job_offer: str) -> str:
        ...
        if any(word in job_lower for word in ["startup", "scale-up", "jeune pousse"]):
            ...
            word in job_lower for word in ["multinational", "groupe", "leader mondial"]
        ):
            return "large"
    ...
    def _get_default_culture(
        self, company_name: str, is_premium_feature: bool = False
    ) -> CompanyCulture:
        ...
        if is_premium_feature:
            return CompanyCulture(
                ...
                tone_recommendations=[
                    ...
                    "Passez Premium pour débloquer l'analyse de culture d'entreprise.",
                ],
    ...
    def get_tone_adaptation_suggestions(self, culture: CompanyCulture) -> List[str]:
        ...
        if culture.innovation_level == "innovative":
            suggestions.append(
                "Mettez en avant votre capacité d'adaptation et d'innovation"
            )
