...
class TrajectoryBuilderService:
    ...
    def create_reconversion_plan(
        ...
        if user_tier == UserTier.FREE:
            logger.info(
                "Trajectory Builder is a premium feature. Returning default for Free user."
            )
            return self._get_fallback_plan(
                user_id, current_role, target_role, is_premium_feature=True
            )
        ...
        try:
            ...
            target_skills = self._analyze_target_skills(
                target_role, target_job_description, user_tier
            )
            ...
            difficulty_level = self._assess_difficulty(
                current_sector, target_sector, skill_gaps
            )
            ...
            estimated_duration = timeline_months or self._estimate_duration(
                difficulty_level, len(skill_gaps)
            )

            # Ressources recommandées
            recommended_resources = self._get_recommended_resources(
                target_sector, skill_gaps
            )
            ...
            logger.info(
                f"Reconversion plan created - Duration: {estimated_duration}m, Success: {success_probability:.1%}"
            )
            return plan
    ...
    def _analyze_target_skills(
        ...
        if job_description:
            ai_skills = self._extract_skills_from_job_description(
                job_description, user_tier
            )
    ...
    def _extract_skills_from_job_description(
        self, job_description: str, user_tier: UserTier
    ) -> List[str]:
        ...
        try:
            response = self.ai_client.generate_content(
                prompt=prompt, temperature=0.3, max_tokens=200, user_tier=user_tier
            )
    ...
    def _identify_skill_gaps(
        self, current_skills: List[str], target_skills: List[str]
    ) -> List[str]:
        ...
        for target_skill in target_skills:
            ...
            if not any(
                target_skill.lower() in current.lower()
                or current.lower() in target_skill.lower()
    ...
    def _assess_difficulty(
        self, current_sector: str, target_sector: str, skill_gaps: List[str]
    ) -> str:
    ...
    def _calculate_success_probability(
        self, current_skills: List[str], target_skills: List[str], difficulty_level: str
    ) -> float:
    ...
    def _generate_trajectory_steps(
        self,
        current_sector: str,
        target_sector: str,
        skill_gaps: List[str],
        ...
        steps.append(
            TrajectoryStep(
                ...
                resources=[
                    ...
                    "Entretiens avec des professionnels du secteur cible",
                ],
                milestones=[
                    "Bilan complet réalisé",
                    "Objectifs définis",
    ...
    def _group_skills_by_learning_path(
    ...
    def _get_learning_resources(self, group_name: str, skills: List[str]) -> List[str]:
        ...
        base_resources = resource_templates.get(
            group_name, ["Formation générale", "Ressources en ligne"]
        )
    ...
    def _estimate_duration(self, difficulty_level: str, gap_count: int) -> int:
        ...
        base_durations = {"easy": 6, "moderate": 9, "challenging": 12, "expert": 18}
    ...
    def _get_recommended_resources(
        self, target_sector: str, skill_gaps: List[str]
    ) -> List[Dict[str, str]]:
        ...
        sector_resources = {
            "tech": [
                {
                    "type": "course",
                    "name": "OpenClassrooms - Développeur",
                    "url": "https://openclassrooms.com",
                },
                {
                    "type": "certification",
                    "name": "Certifications AWS/Azure",
                    "url": "https://aws.amazon.com/certification/",
                },
                {
                    "type": "platform",
                    "name": "GitHub pour portfolio",
            ...
            "finance": [
                ...
                {
                    "type": "course",
                    "name": "Formation comptabilité CNAM",
    ...
    def update_plan_progress(
        self, plan: ReconversionPlan, completed_steps: List[int]
    ) -> ReconversionPlan:
        ...
        try:
            ...
            plan.success_probability = min(
                0.95, plan.success_probability + progress_bonus
            )
    ...
    def _get_fallback_plan(
        ...
        if is_premium_feature:
            return ReconversionPlan(
                ...
                recommended_resources=[
                    {"type": "info", "name": "Passez Premium pour débloquer", "url": ""}
                ],
        ...
        return ReconversionPlan(
            ...
            recommended_resources=[
                {"type": "conseil", "name": "Accompagnement personnalisé", "url": ""}
            ],
