...
class PhoenixSecurityScanner:
    ...
    def __init__(self):
        ...
        self.security_patterns = {
            ...
            "hardcoded_secrets": [
                ...
                (
                    r'(?i)google[_-]?api[_-]?key\s*[=:]\s*["\']([^"\']{20,})["\']',
                    "critical",
                ),
                (
                    r'(?i)openai[_-]?api[_-]?key\s*[=:]\s*["\']([^"\']{20,})["\']',
                    "critical",
                ),
                (
                    r'(?i)(gemini|claude)[_-]?api[_-]?key\s*[=:]\s*["\']([^"\']{20,})["\']',
                    "critical",
                ),
            ],
    ...
    def _should_scan_file(self, file_path: str) -> bool:
        ...
        if any(
            temp in file_path for temp in [".pyc", "__pycache__", ".git", ".DS_Store"]
        ):
            return False
                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        if (
                            ...
                            and target.id.lower() in ["debug", "development"]
    ...
    def _calculate_confidence(self, category: str, matched_text: str) -> float:
        ...
        if any(
            word in matched_text.lower() for word in ["example", "demo", "placeholder"]
        ):
    ...
    def _generate_recommendations(self, issues: List[SecurityIssue]) -> List[str]:
        ...
        if high_critical:
            recommendations.append(
                "🚨 URGENT: Corriger immédiatement les vulnérabilités critiques et hautes"
            )
        ...
        if "hardcoded_secrets" in categories:
            recommendations.append(
                "🔐 Migrer tous les secrets vers variables d'environnement"
            )

        if "sql_injection" in categories:
            recommendations.append(
                "🛡️ Implémenter requêtes paramétrées pour toutes les interactions DB"
            )
    ...
    def _evaluate_compliance(self, issues: List[SecurityIssue]) -> Dict[str, bool]:
        ...
        return {
            ...
            "secrets_management": not any(
                "hardcoded_secrets" in i.category for i in issues
            ),
            "input_validation": not any("sql_injection" in i.category for i in issues),
            "secure_deserialization": not any(
                "unsafe_deserialization" in i.category for i in issues
            ),
            "error_handling": len([i for i in issues if "error_handling" in i.category])
            <= 5,
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", default="security_report.json", help="Output file"
    )
    ...
    print(
        f"Critical: {report.issues_by_severity['critical']}, High: {report.issues_by_severity['high']}"
    )
