    ...
    def validate_file_content(self, content: bytes, file_extension: str) -> None:
        ...
        if file_extension == "txt":
            try:
                ...
                logger.error(f"Unexpected error during TXT file validation: {e}")
                raise ValueError(f"Erreur inattendue lors de la validation TXT: {e}")
            ...
            for pattern in suspicious_pdf_patterns:
                if pattern in content:
                    ...
                    raise ValueError(
                        f"PDF contient des éléments potentiellement dangereux: {pattern.decode()}"
                    )
