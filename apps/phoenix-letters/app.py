...
def render_choice_page():
    ...
    with col2:
        ...
        if st.button(
            ...
            # SECURITY FIX: Do not create predictable guest IDs. A secure session ID will be generated by SecureSessionManager.
...
def _render_developer_tools(settings, initialized_components):
    """Rend les outils de développement (mock API, simulation premium)."""
    st.sidebar.markdown("---")
    ...
    if use_mock:
        ...
        initialized_components["gemini_client"] = MockGeminiClient()
        st.sidebar.warning("Mode Mock API activé.")
    else:
        from infrastructure.ai.gemini_client import GeminiClient
        initialized_components["gemini_client"] = GeminiClient(settings)
...
def render_main_app(current_user, auth_middleware, settings, db_connection, initialized_components):
    ...
    # SECURITY FIX: Explicitly differentiate between a logged-in user and a guest.
    ...
    if not is_guest:
        ...
        # SECURITY FIX: A guest does not have a user_id in the session.
        # The user_tier for guests is always FREE.
        st.session_state.user_tier = UserTier.FREE

    # Render developer tools
    _render_developer_tools(settings, initialized_components)

    # Utilisation des composants initialisés
    generator_page = initialized_components["generator_page"]
    premium_page = initialized_components["premium_page"]
    settings_page = initialized_components["settings_page"]
    about_page = initialized_components["about_page"]
...
def _route_app_pages(current_user, auth_middleware, settings, db_connection):
...
def main():
    ...
    jwt_manager = JWTManager(settings.jwt_secret_key, settings.jwt_algorithm) # Use shared JWTManager
