"""
🧭 ALESSIO NAVIGATION - Navigation inter-applications Phoenix
Composants pour la navigation entre applications avec Alessio intégré.
"""

import streamlit as st
from typing import Optional, Dict, Any
from .config import phoenix_config, ALESSIO_CONTEXTS, get_cross_app_navigation
...
def render_phoenix_navigation(current_app: str, show_iris_links: bool = True):
    ...
    with st.sidebar:
        ...
        for app_name, config in other_apps.items():
            context = ALESSIO_CONTEXTS.get(app_name, {})
            ...
            with col2:
                if show_iris_links and config.iris_enabled:
                    ...
                    if iris_url and st.button("🤖", key=f"iris_{app_name}", help=f"Alessio {context.get('name', 'Assistant')}"):
                        st.markdown(f"""
                        <script>
                        window.open('{iris_url}', '_blank');
                        </script>
                        """, unsafe_allow_html=True)
                        st.info(f"Ouverture d'{context.get('name', 'Alessio')}...")
        
        st.markdown("---")
        
        # Informations sur l'app courante
        current_config = phoenix_config.get_app_config(current_app)
        current_context = ALESSIO_CONTEXTS.get(current_app, {})
        ...
        if current_config:
            ...
            if current_config.iris_enabled:
                st.success(f"🤖 {current_context.get('name', 'Alessio')} disponible")

def render_alessio_app_selector(current_context: str = "phoenix-website"):
    """
    Rend un sélecteur d'application pour Alessio.
    Utilisé dans les interfaces web pour basculer entre les contextes.
    
    Args:
        current_context: Contexte Alessio actuel
    """
    
    st.markdown("### 🤖 Choisir votre assistant Alessio")
    
    # Options disponibles
    alessio_options = {}
    for app_name, context in ALESSIO_CONTEXTS.items():
        if phoenix_config.get_app_config(app_name).iris_enabled:
            alessio_options[app_name] = f"{context['icon']} {context['name']} - {context['specialization']}"
    
    selected_app = st.selectbox(
        "Spécialisation Alessio :",
        options=list(alessio_options.keys()),
        format_func=lambda x: alessio_options[x],
        index=list(alessio_options.keys()).index(current_context) if current_context in alessio_options else 0
    )
    
    if selected_app != current_context:
        st.info(f"Basculement vers {ALESSIO_CONTEXTS[selected_app]['name']}")
        return selected_app
...
def render_phoenix_ecosystem_overview():
    """
    Rend un aperçu de l'écosystème Phoenix avec liens Alessio.
    """
    
    st.markdown("## 🌟 Écosystème Phoenix complet")
    
    cols = st.columns(len(ALESSIO_CONTEXTS))
    
    for i, (app_name, context) in enumerate(ALESSIO_CONTEXTS.items()):
        ...
        with cols[i]:
            ...
            with col_iris:
                if config.iris_enabled and st.button(f"Chat {context['name']}", key=f"chat_{app_name}"):
                    ...
                    st.markdown(f"[🤖 Alessio {context['name']}]({iris_url})")
...
def render_cross_app_alessio_buttons(current_app: str):
    """
    Rend des boutons rapides pour accéder aux autres assistants Alessio.
    
    Args:
        current_app: Application courante
    """
    
    st.markdown("### 🚀 Autres assistants Alessio")
    ...
    for i, (app_name, info) in enumerate(switch_urls.items()):
        ...
        with cols[i % len(cols)]:
            if st.button(
                f"{context.get('icon', '🤖')} {context.get('name', 'Alessio')}",
...
def inject_cross_app_css():
    ...
    .alessio-purple { background: #e9d5ff; color: #7c3aed; }
    .alessio-blue { background: #dbeafe; color: #2563eb; }
    .alessio-green { background: #dcfce7; color: #16a34a; }
    .alessio-orange { background: #fed7aa; color: #ea580c; }
    </style>
    """, unsafe_allow_html=True)