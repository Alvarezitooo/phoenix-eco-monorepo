...
class SupabaseBatchService:
    ...
    def __init__(self, supabase_client, batch_size: int = 10, flush_interval: float = 2.0):
        ...
        logger.info(f"✅ SupabaseBatchService initialized (batch_size={batch_size}, flush_interval={flush_interval}s)")
    ...
    def queue_update(self, table: str, data: Dict[str, Any], filters: Dict[str, Any], callback: Optional[Callable] = None) -> None:
        """
    ...
    def _execute_batch(self, operations: List[BatchOperation]) -> BatchResult:
        ...
        try:
            ...
            logger.info(f"✅ Batch executed successfully: {len(operations)} operations in {execution_time:.2f}s")
        ...
        with self._lock:
            for op in operations:
                if op.retry_count < op.max_retries:
                    ...
                    logger.info(f"🔄 Retrying operation (attempt {op.retry_count}/{op.max_retries})")
                else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batch_success_rate": self._successful_batches / max(self._successful_batches + self._failed_batches, 1),
    ...
    def shutdown(self) -> None:
        ...
        logger.info(f"✅ SupabaseBatchService shutdown complete. Final batch: {final_result.operation_count} operations")


# Instance globale pour l'application
supabase_batch_service = None

def init_batch_service(supabase_client, batch_size: int = 10, flush_interval: float = 2.0) -> SupabaseBatchService:
    """Initialise le service batch global."""
    global supabase_batch_service
    supabase_batch_service = SupabaseBatchService(supabase_client, batch_size, flush_interval)
