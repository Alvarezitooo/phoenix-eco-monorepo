"""
🛡️ SECURITY PATCHES POUR IRIS CROSS-APP INTEGRATION
...
class IrisAppContext(Enum):
    """Contextes d'application validés pour Iris."""
...
class SecureContextBuilder:
    ...
    SECURE_CONTEXT_PREFIXES = {
        IrisAppContext.LETTERS: "[PHOENIX-LETTERS]",
        IrisAppContext.CV: "[PHOENIX-CV]",
        IrisAppContext.RISE: "[PHOENIX-RISE]", 
        IrisAppContext.WEBSITE: "[PHOENIX-WEB]"
    }
    ...
    @classmethod
    def build_secure_contextual_message(cls, message: str, app_context: IrisAppContext) -> str:
        """
        ...
        if not isinstance(app_context, IrisAppContext):
...
class SecureLogger:
    ...
    @classmethod
    def log_iris_interaction(cls, app_context: IrisAppContext, user_id: str, message_preview: str = ""):
        """
        Log sécurisé des interactions Iris.
        ...
        # Log sécurisé
        logger.info(
            f"Iris interaction - App: {anonymized_context}, "
            f"User: {anonymized_user_id}, "
            f"Preview: {safe_message_preview}"
        )
    
    @classmethod
    def log_security_event(cls, event_type: str, app_context: IrisAppContext, details: Dict[str, Any]):
        """
    ...
    @classmethod
    def _anonymize_app_context(cls, app_context: IrisAppContext) -> str:
        """Anonymise le contexte d'application."""
        context_map = {
            IrisAppContext.LETTERS: "L***",
            IrisAppContext.CV: "C***", 
            IrisAppContext.RISE: "R***",
            IrisAppContext.WEBSITE: "W***"
        }
...
class SecureIrisClient:
    """
    Client Iris sécurisé avec les corrections appliquées.
    Intègre les corrections VUL-001 et VUL-002.
    """
    
    def __init__(self, app_context: IrisAppContext, user_id: str, api_url: str):
        """
    ...
    def send_secure_message(self, message: str, additional_context: Optional[Dict[str, Any]] = None) -> str:
        """
        Envoie un message sécurisé à Iris.
...
def create_secure_iris_client(app_context: str, user_id: str, api_url: str) -> SecureIrisClient:
    """
    Crée un client Iris sécurisé à partir d'un contexte string.
    Fonction de transition pour compatibilité.
    """
    # Mapping string -> enum pour migration
    context_mapping = {
        "phoenix-letters": IrisAppContext.LETTERS,
        "phoenix-cv": IrisAppContext.CV,
        "phoenix-rise": IrisAppContext.RISE,
        "phoenix-website": IrisAppContext.WEBSITE
    }
