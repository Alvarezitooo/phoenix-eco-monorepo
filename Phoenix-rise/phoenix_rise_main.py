...
class SessionStateStorage:
    ...
    def __init__(self, user_id: str):
        self.user_key_prefix = f"phoenix_rise_{hashlib.sha256(user_id.encode()).hexdigest()[:10]}"
    ...
    def save_data(self, data_type: str, data: Any):
        ...
        serializable_data = [asdict(item) for item in data] if isinstance(data, list) and data and hasattr(data[0], '__dict__') else data
        st.session_state[key] = serializable_data

    def load_data(self, data_type: str, default_value: Any = None) -> Any:
        key = self._get_key(data_type)
        return st.session_state.get(key, default_value if default_value is not None else [])
...
class CoachingService:
    ...
    """
    @staticmethod
    def generate_ai_encouragement(mood_score: int, confidence_score: int, notes: str) -> str:
    ...
    @staticmethod
    def generate_interview_feedback(response: str, question_type: str) -> Dict[str, Any]:
        # PRÊT POUR L'IA : Ce prompt est conçu pour l'API Gemini
        prompt = f"""
...
class QuestionBank:
    ...
    QUESTIONS = {
        "cybersécurité": [{"question": "Comment votre expérience en EHPAD vous a-t-elle préparé à la gestion de crise en cybersécurité ?", "type": "reconversion"}],
        "développement": [{"question": "Décrivez un projet où vous avez dû apprendre rapidement une nouvelle compétence, comme vous le faites pour le code.", "type": "apprentissage"}],
        "marketing": [{"question": "Comment votre capacité d'empathie, développée en tant qu'aide-soignant, peut-elle être un atout pour comprendre les besoins d'un client ?", "type": "reconversion"}]
    }
...
class MoodJournalManager:
    ...
    def add_mood_entry(self, mood_score: int, energy_level: int, confidence_level: int, notes: str) -> str:
        entry = MoodEntry(
            date=datetime.now().strftime("%Y-%m-%d"),
            mood_score=mood_score, energy_level=energy_level, confidence_level=confidence_level,
            notes=notes, tags=[]
        )
        ...
        return self.coach_service.generate_ai_encouragement(mood_score, confidence_level, notes)

    def get_mood_analytics(self) -> Optional[Dict]:
        mood_entries_raw = self.storage.load_data("mood_entries", [])
        if not mood_entries_raw: return None
        
        mood_entries = [MoodEntry(**e) for e in mood_entries_raw]
        avg_mood = sum(e.mood_score for e in mood_entries) / len(mood_entries)
        avg_confidence = sum(e.confidence_level for e in mood_entries) / len(mood_entries)
        
        return {
            "avg_mood": round(avg_mood, 1),
            "avg_confidence": round(avg_confidence, 1),
            "trend": "📈" if len(mood_entries) > 1 and mood_entries[-1].mood_score > mood_entries[0].mood_score else "➡️"
        }
...
def render_mood_tracker(journal_manager: MoodJournalManager):
    ...
    with st.container(border=True):
        ...
        notes = st.text_area("💭 Notes (ex: 'J'ai postulé à 3 offres', 'Entretien demain')", height=100)

        if st.button("Enregistrer et recevoir mon encouragement"):
            encouragement = journal_manager.add_mood_entry(mood, energy, confidence, notes)
...
def render_dashboard(journal_manager: MoodJournalManager):
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown(f"<div class='metric-card'><h4>Humeur Moyenne</h4><h2>{analytics['avg_mood']}/10</h2></div>", unsafe_allow_html=True)
    with col2:
        st.markdown(f"<div class='metric-card'><h4>Confiance Moyenne</h4><h2>{analytics['avg_confidence']}/10</h2></div>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<div class='metric-card'><h4>Tendance</h4><h2>{analytics['trend']}</h2></div>", unsafe_allow_html=True)

def render_coaching_interface(coach_service: CoachingService):
    st.subheader("🎯 Coach d'Entretien IA")
    with st.container(border=True):
        sector = st.selectbox("Choisissez le secteur de l'entretien :", ["cybersécurité", "développement", "marketing"])
        ...
        if 'current_question' in st.session_state:
            ...
            with st.form("response_form"):
                ...
                if submitted and response:
                    feedback = coach_service.generate_interview_feedback(response, question_data['type'])
                    st.markdown("---")
                    st.success(f"**Point Fort :** {feedback['strength']}")
                    st.warning(f"**Axe d'amélioration :** {feedback['improvement']}")
                    st.progress(feedback['score'] / 10, text=f"Score de la réponse : {feedback['score']}/10")
...
def main():
    ...
    user_id = st.sidebar.text_input("Entrez votre email pour commencer", "matthieu.alvarez@example.com")
    ...
    tab1, tab2, tab3 = st.tabs(["**📊 Mon Suivi Quotidien**", "**🎯 Coaching Entretien**", "**📖 Mon Journal**"])
    ...
    with tab3:
        ...
        st.image("https://img.freepik.com/free-vector/man-thinking-with-light-bulb-cartoon-icon-illustration_138676-2794.jpg", caption="Bientôt disponible !")
